...
class ParserService:
    ...
    def get_p_educations_and_p_group_ids(self, parser: Parser) -> tuple[int, int]:
        ...
        if parser.platform_id == 1:
            try:
                logging.warning(
                    f"{datetime.datetime.now()} Запрос на сервер в get_p_educations_and_p_group_ids. User id: {parser.student_id}"
                )
                r = requests.get(
    ...
    def create_parser(self, student_id: int, parser_data: ParserCreate):
        ...
        if parser_data.platform_id == 1:
            ...
            try:
                logging.warning(f"{datetime.datetime.now()} Запрос на сервер в create_parser. User id: {student_id}")
                r = requests.post(
