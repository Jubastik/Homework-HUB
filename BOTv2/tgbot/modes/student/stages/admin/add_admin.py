...
from services.restapi.restapi import get_class, appoint_administrator
from services.restapi.api_error import ApiError


class AddAdmin(Stage):
    ...
    async def handle_callback(self, call: CallbackQuery) -> bool:
        if "student_name:" in call.data:
            ...
            res = await appoint_administrator(tgid)
            if isinstance(res, ApiError):
                # TODO
                return res
            await call.answer(process_text(TextKeys.admin_added))
